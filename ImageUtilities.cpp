...
QSize qsizeFromCVSize(const cv::Size &sz, double scale)
{
    return QSize(sz.width * scale,sz.height * scale);
}

QPoint qpointFromCVPoint(const cv::Point &pt, double scale)
{
    return QPoint( pt.x * scale, pt.y * scale);
}
